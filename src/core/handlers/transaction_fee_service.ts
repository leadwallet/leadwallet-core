...
export class TransactionFeeService {
    ...
    feeResponse.body.meta?.error.message || sizeResponse.body.meta?.error.message || "Couldn't get txn fee/size"
		...
		return Promise.resolve({
			...
			"slow_fee": ticker === "doge" ? parseFloat(feeResponse.body.payload.slow_fee_per_byte) * sizeInBytes * 2 : (parseFloat(feeResponse.body.payload.slow_fee_per_byte) * sizeInBytes) / 2,
			"standard_fee": ticker === "doge" ? parseFloat(feeResponse.body.payload.standard_fee_per_byte) * sizeInBytes * 2 : (parseFloat(feeResponse.body.payload.standard_fee_per_byte) * sizeInBytes) / 2,
			"fast_fee": ticker === "doge" ? parseFloat(feeResponse.body.payload.fast_fee_per_byte) * sizeInBytes * 2 : (parseFloat(feeResponse.body.payload.fast_fee_per_byte) * sizeInBytes) / 2,
   "unit": ticker,
